...
class TwitterSentimentAnalysis {
    ...
    public function getTweets() {
        try
        {
            ...
            $stmt = $bdd->prepare("SELECT * FROM tweets LEFT JOIN author_tweet ON tweets.id_author_tweet = author_tweet.id_author_tweet LIMIT 3");
            $stmt->execute();
...
//            $sentiment=$DatumboxAPI->SentimentAnalysis($tweet['tweet_text']); //call Datumbox service to get the sentiment
//            $date = new DateTime($tweet['tweet_created_at']);
//            $date->setTimezone(new DateTimeZone('Europe/Paris')); // +02
